...
const POS = {
    ...
    updateItem: function(index, field, value) {
        ...
        // Dynamically update the total cell in the DOM to avoid losing focus
        const row = document.querySelector(`#pos-cart-items tr:nth-child(${index + 1})`);
        if (row) {
            const totalCell = row.cells[4];
            if (totalCell) {
                totalCell.innerText = (AppState.settings.currency || '₹') + this.cart[index].total.toFixed(2);
            }
        }
    ...
    finalizePrint: function() {
        ...
        const validItems = this.cart.filter(item => item.qty > 0);
        if(validItems.length === 0) {
            alert("Cart must contain at least one item with a quantity greater than zero!");
            return;
        }
        ...
        const bill = {
            ...
            splitCash: splitCash,
            splitUpi: splitUpi,
            staffName: sessionStorage.getItem('userName') || 'VIKI',
            counterId: 'Counter 1'
        };
