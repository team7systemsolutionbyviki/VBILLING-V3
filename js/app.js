...
const app = {
    ...
    login: function() {
        ...
        // Staff Operator validation
        if (typeof ShopManager !== 'undefined') {
            const shops = ShopManager.getShops();
            for (const shop of shops) {
                const staffJSON = localStorage.getItem('vmasterbilling_' + shop.id + '_staff');
                const staffList = staffJSON ? JSON.parse(staffJSON) : [];
                const matchingStaff = staffList.find(st => st.username.toLowerCase() === email.toLowerCase());
                
                if (matchingStaff) {
                    if (matchingStaff.password === pass) {
                        if (matchingStaff.status !== 'Active') {
                            alert('This staff account is currently inactive. Contact your Shop Administrator.');
                            return;
                        }
                        
                        // Check AMC Expiration
                        const todayStr = new Date().toISOString().split('T')[0];
                        const expiryDate = shop.amcExpiryDate || todayStr;
                        if (expiryDate < todayStr) {
                            alert('This shop\'s subscription has expired. Please contact Shop Administrator.');
                            return;
                        }
                        
                        sessionStorage.setItem('isLoggedIn', 'true');
                        sessionStorage.setItem('userRole', 'Staff');
                        sessionStorage.setItem('userName', matchingStaff.name);
                        sessionStorage.setItem('staffUsername', matchingStaff.username);
                        sessionStorage.setItem('staffRole', matchingStaff.role);
                        sessionStorage.setItem('currentShopId', shop.id);
                        sessionStorage.setItem('currentShopName', shop.name);
                        ...
                        alert('Invalid Password for Staff Account!');
                        return;
                    }
                }
            }
        ...
        // Global / Seeded Staff validation
        const globalStaffJSON = localStorage.getItem('vmasterbilling_staff');
        const globalStaffList = globalStaffJSON ? JSON.parse(globalStaffJSON) : [];
        const matchingGlobalStaff = globalStaffList.find(st => st.username.toLowerCase() === email.toLowerCase());
        if (matchingGlobalStaff) {
            if (matchingGlobalStaff.password === pass) {
                if (matchingGlobalStaff.status !== 'Active') {
                    alert('This staff account is currently inactive.');
                    return;
                }
                sessionStorage.setItem('isLoggedIn', 'true');
                sessionStorage.setItem('userRole', 'Staff');
                sessionStorage.setItem('userName', matchingGlobalStaff.name);
                sessionStorage.setItem('staffUsername', matchingGlobalStaff.username);
                sessionStorage.setItem('staffRole', matchingGlobalStaff.role);
                sessionStorage.removeItem('currentShopId');
                sessionStorage.removeItem('currentShopName');
    ...
    loadPage: function(page) {
        ...
        if (userRole === 'Staff') {
            if (staffRole === 'Cashier' && page !== 'dashboard' && page !== 'pos') {
                alert("Access Denied: Cashiers are restricted to Dashboard and Patti Bill (POS) operations.");
                this.loadPage('pos');
                return;
            }
            if (staffRole === 'Manager' && page === 'settings') {
                alert("Access Denied: Managers do not have access to shop settings.");
                this.loadPage('dashboard');
                return;
            }
        }
...
<script>window.onafterprint = function() { window.close(); }; window.addEventListener('afterprint', function() { window.close(); }); setTimeout(function() { window.print(); setTimeout(function() { window.close(); }, 500); }, 300);<\/script>
    ...
    initSettings: function() {
        ...
        // Render staff operator accounts
...
<div class="no-print" style="position:fixed;top:10px;right:10px;z-index:9999;">
  ...
  <\/script>
    ...
    updateSidebarVisibility: function() {
        ...
        if (userNameSpan) {
            let uName = sessionStorage.getItem('userName') || 'User';
    ...
    renderStaffList: function() {
        ...
        if (staff.length === 0) {
            tbody.innerHTML = `<tr><td colspan="6" class="text-center text-muted py-3">No staff accounts created yet. Click "Add New Staff" to create.</td></tr>`;
            return;
        }
        
        tbody.innerHTML = staff.map(st => {
            const statusBadge = st.status === 'Active' ? '<span class="badge bg-success">Active</span>' : '<span class="badge bg-danger">Inactive</span>';
            const roleBadge = st.role === 'Manager' ? '<span class="badge bg-info text-dark">Manager</span>' : 
                             st.role === 'Cashier' ? '<span class="badge bg-primary text-white">Cashier</span>' : 
                             '<span class="badge bg-secondary text-white">Loader</span>';
            return `
                <tr>
                    <td class="fw-bold">${st.name}</td>
                    <td class="font-monospace">${st.username}</td>
                    <td>${roleBadge}</td>
                    <td class="font-monospace">${st.phone || '-'}</td>
                    <td>${statusBadge}</td>
                    <td class="text-center">
                        <button class="btn btn-sm btn-outline-warning text-dark me-1" onclick="app.openStaffModal('${st.id}')"><i class="fas fa-edit"></i></button>
                        <button class="btn btn-sm btn-outline-danger" onclick="app.deleteStaffMember('${st.id}')"><i class="fas fa-trash"></i></button>
                    </td>
                </tr>
            `;
        }).join('');
    ...
    openStaffModal: function(id = '') {
        ...
        if (id) {
            if (modalTitle) modalTitle.innerHTML = `<i class="fas fa-user-edit me-2"></i> Edit Staff Account`;
            const staffList = LocalDB.getStaff() || [];
            const st = staffList.find(s => s.id === id);
            if (st) {
                document.getElementById('staff-id').value = st.id;
                document.getElementById('staff-name').value = st.name;
                document.getElementById('staff-username').value = st.username;
                if (usernameInput) usernameInput.readOnly = true;
                document.getElementById('staff-password').value = st.password;
                document.getElementById('staff-role-select').value = st.role || 'Cashier';
                document.getElementById('staff-status').value = st.status || 'Active';
                document.getElementById('staff-phone').value = st.phone || '';
            }
        } else {
            if (modalTitle) modalTitle.innerHTML = `<i class="fas fa-user-plus me-2"></i> Add New Staff Account`;
    ...
    saveStaffMember: function() {
        ...
        if (currentShopId) {
            ...
            if (shop && shop.adminUsername.toLowerCase() === username) {
                alert(`Username "${username}" is already in use by the Shop Administrator!`);
                return;
            }
        }
        ...
        if (!id) {
            if (staffList.some(s => s.username === username)) {
                alert(`Username "${username}" already exists in this shop!`);
                return;
            }
            ...
            if (idx > -1) {
                ...
                staffList[idx].password = password;
    ...
    deleteStaffMember: function(id) {
        if (confirm('Are you sure you want to permanently delete this staff member?')) {
            let staffList = LocalDB.getStaff() || [];
